...
function initPaperViewer() {
    ...
    // Read Paper button functionality
    const readBtn = document.querySelector('.read-btn');
    if (readBtn) {
        readBtn.addEventListener('click', () => {
            // Scroll to the book viewer
            const paperViewer = document.querySelector('.paper-viewer');
            if (paperViewer) {
                paperViewer.scrollIntoView({ behavior: 'smooth', block: 'center' });
                
                // Automatically flip the page after scrolling
                setTimeout(() => {
                    if (!isFlipped) {
                        turnBtn.click();
                    }
